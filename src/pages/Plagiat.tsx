...
import { Search, FileText, Upload, ArrowLeft, CheckCircle, AlertCircle, AlertTriangle, Download, Loader2 } from 'lucide-react';
...
export default function Plagiat() {
  ...
  const getRiskLevel = (similarite?: number) => {
    if (!similarite) return { color: 'slate', label: 'Non déterminé', icon: AlertCircle };
    if (similarite < 15) return { color: 'green', label: 'Faible risque', icon: CheckCircle };
    if (similarite < 40) return { color: 'amber', label: 'Risque modéré', icon: AlertTriangle };
      ...
      <div className="max-w-3xl mx-auto px-4">
        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            ...
            <p className="text-slate-600">Analysez votre contenu pour détecter les similitudes</p>
          </div>
        </div>
        ...
        {!result ? (
          <div className="card p-8">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {inputMethod === 'text' && (
                <div>
                  <label htmlFor="texte" className="label">Texte à vérifier :</label>
                  <textarea
                    id="texte"
                    name="texte"
                    rows={10}
              ...
              {inputMethod === 'file' && (
                <div>
                  <label htmlFor="fichier" className="label">Fichier à analyser :</label>
                    ...
                    <div className="h-40 border-2 border-dashed border-slate-300 rounded-lg flex flex-col items-center justify-center gap-3 hover:border-[#009639] hover:bg-[#009639]/5 transition-all">
                      ...
                      {selectedFile ? (
                        <div className="text-center">
                          ...
                          <p className="text-xs text-slate-500 mt-1">{(selectedFile.size / 1024).toFixed(1)} Ko</p>
                        </div>
                      ) : (
                        <div className="text-center">
                          <p className="text-sm text-slate-500">Cliquez ou glissez votre fichier ici</p>
                          <p className="text-xs text-slate-400 mt-1">Formats: TXT, PDF, DOCX</p>
                        </div>
                      )}
                    </div>
                  </div>
            ...
            <div className="card p-8 border-l-4 border-l-[#009639]">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  ...
                  <p className="text-slate-600">Voici les résultats de l'analyse</p>
                </div>
              </div>
                ...
                <div className="p-6 bg-slate-50 rounded-xl mb-6">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span className={`text-2xl font-bold ${
                      getRiskLevel(result.similarite).color === 'green' ? 'text-green-600' :
                      getRiskLevel(result.similarite).color === 'amber' ? 'text-amber-600' :
                      getRiskLevel(result.similarite).color === 'red' ? 'text-red-600' : 'text-slate-600'
                    }`}>
                      {result.similarite}%
                    </span>
                  </div>
                  ...
                  <div className="w-full h-3 bg-slate-200 rounded-full overflow-hidden mb-3">
                    <div 
                      className={`h-full transition-all ${
                        getRiskLevel(result.similarite).color === 'green' ? 'bg-green-500' :
                        getRiskLevel(result.similarite).color === 'amber' ? 'bg-amber-500' :
                        getRiskLevel(result.similarite).color === 'red' ? 'bg-red-500' : 'bg-slate-400'
                      }`}
                      ...
                      return (
                        <>
                          <Icon className={`w-5 h-5 ${
                            ...
                            risk.color === 'red' ? 'text-red-600' : 'text-slate-600'
                          }`} />
        ...
        <div className="mt-6 p-6 bg-[#009639]/5 rounded-xl border border-[#009639]/20">
          <p className="text-sm mb-3">Vous avez une œuvre originale à protéger ?</p>
