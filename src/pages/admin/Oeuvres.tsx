...
export default function Oeuvres() {
  ...
  // Liste unique des auteurs pour le select
  const auteurs = Array.from(
    new Set(oeuvres.map((o) => o.auteur).filter((a) => a && a.trim() !== ""))
  );
  ...
  const filteredOeuvres = oeuvres.filter((o) => {
    ...
    const matchesAuteur = filterAuteur ? o.auteur === filterAuteur : true;
    return matchesSearch && matchesType && matchesAuteur;
  });
  ...
  return (
    <div className="space-y-8">
      <h1 className="text-2xl font-extrabold text-slate-900">Liste des Œuvres</h1>
            ...
            <table className="min-w-full text-sm text-slate-700">
              <thead>
                <tr className="bg-slate-100 text-slate-800">
                  ...
                  <th className="px-4 py-3 text-left font-semibold">Date enregistrement</th>
                  <th className="px-4 py-3 text-left font-semibold">IA détectée</th>
              ...
              <tbody className="divide-y divide-slate-200">
                {paginatedOeuvres.map((o) => (
                  <tr key={o.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3">{o.type_oeuvre}</td>
                    ...
                    <td className="px-4 py-3 text-slate-500">{o.fichier_nom}</td>
                    <td className="px-4 py-3 text-slate-500">{o.date_creation || "-"}</td>
                    <td className="px-4 py-3 text-slate-500">{o.date_enregistrement || "-"}</td>
                    <td className="px-4 py-3">{o.ia_detectee || "-"}</td>
                    <td className="px-4 py-3">{o.score_ia || "-"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="p-4 bg-slate-100 rounded-lg text-center">
              ...
              <p className="text-2xl font-bold">
                {oeuvres.filter((o) => o.type_oeuvre?.toLowerCase() === "musique").length}
              </p>
            </div>
            <div className="p-4 bg-slate-100 rounded-lg text-center">
              <p className="text-sm text-slate-500 flex items-center justify-center gap-1">
              ...
              <p className="text-2xl font-bold">
                {oeuvres.filter((o) => o.type_oeuvre?.toLowerCase() === "texte").length}
              </p>
            </div>
          </div>
        </div>
